...
본 프로토콜은 1인 설계자(deundeuni)의 사유에서 출발했다. 공장에서 기계를 만지며, 건설노동자로 일하며 쌓인 현장 지식에서 시작했다.

soma-moa는 그 사유를 글로벌 규격 코드로 정리한 기록이다.

이 정리 과정에서 구글의 Transformer(2017), TensorFlow(2015), Gemma(2024), Meta의 Llama(2023~) 등 개방된 기반 모델을 검증 도구로 활용할 수 있었음에 감사한다. Meta AI와 Gemini는 검증 도구이며, 설계의 착상은 deundeuni에게 있다.
---
